...
def main():
    ...
    dataset_type = 2
    datasets = ["CelebA_gender","CelebA_race","EyePACS"]
    '''
    0 - CelebA_gender
    1 - CelebA_race
    2 - EyePACS
    ...
    for beta in betas:
        ...
        for epoch in range(epochs):
            ...
            for x, y, a in tqdm(dataloader,disable=not(debugging)):
                ...
                #IB loss
                if method == 0:
                    loss = cost_functions.get_IB_or_Skoglund_loss(yhat, y, mu, logvar, beta)
                #Skoglund loss
                elif method == 1:
                    loss = Skoglund_loss = cost_functions.get_IB_or_Skoglund_loss(yhat_fair,y,mu,logvar,beta)
                #Combined loss
                elif method == 2:
                    loss = Combined_loss = cost_functions.get_combined_loss(yhat,yhat_fair,y,mu,logvar,beta)
            ...
            # evaluations.evaluate(model,test_set,batch_size,numworkers,fair,beta,epoch,debugging,device,"Test")
